...
//            $xmlfomart = htmlentities($xml);
    ...
    public function importAndPublish(){
        ...
        $cusPhone = $this->input->post('tel') ? $this->input->post('tel') : '';
        ...
        foreach ($datas as $item) { //UNIT_AMT
            if(is_array($item)){
                ...
                */
                ...
                $item['ATM'] = (int)str_replace(",", "", $item['Price']); 
                unset($item['Price']);

                $strFinal .= $this->strReplaceAssoc($item, $product_content);
                $strFinal = str_replace('>Price<', '>'.$item['ATM'].'<', $strFinal);
            }
        }
...
EOT;
    ...
    public function downloadInvPDF(){
        $pattern = $this->input->post('pattern') ? $this->input->post('pattern') : '';
        $serial = $this->input->post('serial') ? $this->input->post('serial') : '';
        $number = $this->input->post('invno') ? $this->input->post('invno') : '';
